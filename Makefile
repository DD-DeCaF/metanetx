...
pip-compile:
	docker-compose run --rm web pip-compile --generate-hashes dev-requirements.in
	docker-compose run --rm web pip-compile --generate-hashes requirements.in
...
flake8:
	docker-compose run --rm web \
		flake8 src/metanetx tests

## Check Python package import order.
isort-check:
	docker-compose run --rm web \
		isort --check-only --recursive src/metanetx tests

## Sort imports and write changes to files.
isort:
	docker-compose run --rm web \
		isort --recursive src/metanetx tests

## Verify source code license headers.
license:
	./scripts/verify_license_headers.sh src/metanetx tests
